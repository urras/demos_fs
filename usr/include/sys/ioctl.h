...
/***** флаги для RW - "Винчестерский" диск *******/

#define RWIO_CMP   (('r'<<8)|0) /*установка предкомпенсации*/
#define RWIO_WCON  (('r'<<8)|1) /*включение проверки запмси*/
#define RWIO_WCOFF (('r'<<8)|2) /*выключение проверки запмси*/

/***** FD - гибкий диск *******/
/***** структура данных для ioctl форматирования *******/

struct FD_FORMAT_BOX            /* параметры форматирования */
{
int     sizetrk;                /* тип дорожки (размер блока) =2 для 512 */
int     firsttrk;               /* начальная дорожка */
int     numbtrk;                /* колличество дорожек */
};
...
struct lpmodes {
	short   lpm_flags;              /* флаги */
	short   lpm_maxcol;             /* ширина строки */
	short   lpm_ejline;             /* число строк */
	short   lpm_inch;               /* размер страницы в дюймах */
	short   lpm_skip;               /* пропуск на сгиб страницы */
	short   lpm_shrift;             /* шрифт */
	short   lpm_vint;               /* интервал */
	short   lpm_left;               /* левое поле */
};

/* Флаги */
#define LPOPEN      01   /* АЦПУ занято */
#define LPON        02   /* АЦПУ включено */
#define LPCAP       04   /* Есть маленькие буквы */
#define LPRAW      010   /* прозрачный вывод */
#define LPCYRILL   020   /* Есть кириллица */
#define LPSISO     040   /* SISO - стиль переключения регистров */
#define LPZNAK    0100   /* Если LPCAP - есть ли большой твердый знак,
				 иначе - есть ли вообще твердый знак */
#define LPCLEAR   0200   /* Очистка буфера и состояния */
#define LPROB     0400   /* принтер типа "Robotron 6329" */
#define LPLX     01000   /* принтер типа "Epson LX-800" */
#define LPFX     02000   /* принтер типа "Epson FX-800" */
#define LPD100   04000   /* принтер типа "D100-M" */
#define LPINIT  010000   /* Инициализация печати*/
#define LPHTB   020000   /* Замена табуляции пробелами*/
#define LPINCH  040000   /* Установка длины страницы в дюймах*/
#define LPNLQ  0100000   /* Выбор качественного шрифта*/
